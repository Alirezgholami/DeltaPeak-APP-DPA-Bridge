# DPA v0.4.1 Backend/Auth Contract
...
## Session reconciliation and activity — v0.4.1

- `GET /v1/auth/me` — authenticated profile revalidation. Flutter calls this in the background after restoring Secure Storage and on resume.
- `POST /v1/auth/activity` — authenticated heartbeat. Needed because DPA peak browsing is offline-first; without this endpoint `last_activity_at` would only move on login/refresh/admin calls.
- `GET /ready` — verifies database connectivity for deployment readiness probes.
- SMS production providers: generic `webhook` or direct `kavenegar` VerifyLookup. Provider credentials are backend-only secrets.
